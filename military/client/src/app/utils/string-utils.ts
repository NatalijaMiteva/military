export class StringUtils {
  ...
  public static isNumeric(str: any): boolean {
    ...
    // From jQuery
    // (https://github.com/jquery/jquery/blob/25d8ccd1112d75394b91071ff7eba13283aaf898/src/core.js#L223)
    return !isNaN(str - parseFloat(str));
  }

  public static startsWith(str: string | null, search: string | null): boolean {
    if (!str || !search) {
      return false;
    }
  ...
  public static coalesce(...strings: string[]): string | null {
    for (const str of strings) {
      if (!StringUtils.isEmpty(strings[str])) {
        return strings[str];
      }
    }
    return null;
  ...
  public static deepCompare(x: any, y: any): boolean {
    let p;
    const leftChain: any[] = [], rightChain: any[] = [];
  ...
  public static format(format: string, ...strings: string[]) {
    return format.replace(/{(\d+)}/g, (match, index) =>
      typeof strings[index] !== 'undefined'
        ? strings[index]
        : match);
  }
}
